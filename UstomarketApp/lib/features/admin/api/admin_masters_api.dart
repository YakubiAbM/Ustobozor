...
class AdminMastersCrudApi {
  ...
  static Future<AdminMasterDetail> approve(int masterId, {String note = ''}) async {
    final res = await AdminHttp.post(
      '/admin/api/masters/$masterId/approve',
      body: {'note': note},
    );
    if (res.statusCode != 200) {
      throw Exception('Не удалось одобрить');
    }
    return AdminMasterDetail.fromJson(
      jsonDecode(utf8.decode(res.bodyBytes)) as Map<String, dynamic>,
    );
  ...
  static Future<AdminMasterDetail> reject(int masterId, {String note = ''}) async {
    final res = await AdminHttp.post(
      '/admin/api/masters/$masterId/reject',
      body: {'note': note},
    );
    if (res.statusCode != 200) {
      throw Exception('Не удалось отклонить');
    }
    return AdminMasterDetail.fromJson(
      jsonDecode(utf8.decode(res.bodyBytes)) as Map<String, dynamic>,
    );
