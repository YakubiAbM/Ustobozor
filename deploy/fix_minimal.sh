...
echo ""
C=/etc/letsencrypt/live/ustomarket.tj/fullchain.pem
K=/etc/letsencrypt/live/ustomarket.tj/privkey.pem
[[ -f $C ]] || C=/etc/ssl/certs/ustomarket-selfsigned.crt
[[ -f $K ]] || K=/etc/ssl/private/ustomarket-selfsigned.key
[[ -f $C ]] || { mkdir -p /etc/ssl/private; openssl req -x509 -nodes -days 365 -newkey rsa:2048 -keyout /etc/ssl/private/ustomarket-selfsigned.key -out /etc/ssl/certs/ustomarket-selfsigned.crt -subj /CN=ustomarket.tj; C=/etc/ssl/certs/ustomarket-selfsigned.crt; K=/etc/ssl/private/ustomarket-selfsigned.key; }
cat > /etc/nginx/sites-available/ustomarket << EOF
upstream ustomarket_app { server 127.0.0.1:8000; }
server {
    listen 80; listen [::]:80;
    server_name ustomarket.tj www.ustomarket.tj 185.185.142.229;
...
curl -sI --max-time 10 https://ustomarket.tj/products?limit=1 | head -3
echo DONE
